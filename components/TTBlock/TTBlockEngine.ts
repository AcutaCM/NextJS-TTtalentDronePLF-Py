...
export class TTBlockEngine {
  private context: ExecutionContext;
  private nodes: TTBlockNode[];
  private edges: TTBlockEdge[];
  ...
  private async executeNode(node: TTBlockNode): Promise<void> {
  ...
  private async executeNodeLogic(node: TTBlockNode): Promise<any> {
    const { type, data } = node;
    const params = data.parameters || {};
    
    // 优先使用 data.nodeType，如果不存在则使用 type
  ...
  private async executeTakePhoto(params: any): Promise<any> {
    ...
    const filename = params.filename?.replace('{timestamp}', Date.now().toString()) || `photo_${Date.now()}.jpg`;
    const result = {
      status: 'success',
      filename,
  ...
  private async executeAIDetection(params: any): Promise<any> {
    ...
    // 模拟检测结果
    const detections = [];
    const numDetections = Math.floor(Math.random() * (params.maxDetections || 5));
    
    for (let i = 0; i < numDetections; i++) {
      detections.push({
        class: `object_${i}`,
        confidence: Math.random() * (1 - (params.confidence || 0.7)) + (params.confidence || 0.7),
        bbox: {
          x: Math.random() * 800,
          y: Math.random() * 600,
  ...
  // 获取下一个要执行的节点
  private async getNextNodes(currentNode: TTBlockNode, result: any): Promise<TTBlockNode[]> {
    const outgoingEdges = this.edges.filter(edge => edge.source === currentNode.id);
    const nextNodes: TTBlockNode[] = [];
